                ...
                using (var connection = new NpgsqlConnection(_connectionString))
                {
                    ...
                    var sqlOffer = @$"select distinct aa.partner_id,
                               ...
                               where o.branch_id = aa.branch_id) ordersNumbers
                                ...
                                where public.ST_Intersects(public.ST_GeomFromText('POINT({longitude} {latitude})', 4326), aa.geometry::geometry) and aa.active = true and
        ...
        public OfferProducts GetProductOffersByLocationPoint(string latitude, string longitude, FilterOffer filter)
        {
            try
            {
                ...
                using (var connection = new NpgsqlConnection(_connectionString))
                {
                    ...
                    sql += @$"where public.ST_Intersects(public.ST_GeomFromText('POINT({longitude} {latitude})', 4326), aa.geometry::geometry) and aa.active = true
            ...
            try
            {
                using (var connection = new NpgsqlConnection(_connectionString))
                {
                    var sql = $@"select aa.name
                        ...
                        where public.ST_Intersects(public.ST_GeomFromText('POINT({longitude} {latitude})', 4326), aa.geometry::geometry)
        ...
        public OfferFilters GetFiltersByLocationPoint(string latitude, string longitude, string filter)
        {
            try
            {
                using (var connection = new NpgsqlConnection(_connectionString))
                {
                    ...
                    join catalog.category c on c.category_id = cbp.category_id
                    where public.ST_Intersects(public.ST_GeomFromText('POINT({longitude} {latitude})', 4326), aa.geometry::geometry)and
