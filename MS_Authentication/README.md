...
Microserviço responsável pela **autenticação e autorização** de usuários na plataforma PAM. Gerencia login, logout, tokens JWT, recuperação de senha, controle de acesso baseado em roles (RBAC), autenticação multi-fator (MFA) e integração com provedores externos.
...
- 🔑 **Recuperação de Senha**: Reset via email/SMS
- 📱 **OTP**: Autenticação de dois fatores
- ⚙️ **Configurações**: Políticas de senha e segurança
- 👥 **Colaboradores**: Gestão de equipe interna
- 🔍 **Auditoria**: Log de ações de segurança
...
```bash
git clone https://github.com/EmmanuelSMenezes/PAM_MS_Authentication.git
cd PAM_MS_Authentication
```
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=localhost;Database=PAM_PAM_MS_Authentication;Trusted_Connection=true;MultipleActiveResultSets=true"
  }
}
```
...
```bash
# Build da imagem
docker build -t pam_ms_authentication .
...
- **Produção**: `https://api.pam.com/pam_ms_authentication/swagger`
...
- **🐛 Issues**: [GitHub Issues](https://github.com/EmmanuelSMenezes/PAM_MS_Authentication/issues)
- **📖 Wiki**: [Documentação Completa](https://github.com/EmmanuelSMenezes/PAM_MS_Authentication/wiki)
...
**[⬆ Voltar ao Topo](#-PAM_MS_Authentication)**
